			...
			//try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						...
						Sql.AppendParameter(cmd, s$firsttextfield$, Sql.SqlFilterMode.StartsWith, "$firsttextfield$");
			...
			try
			{
				...
				using ( IDbConnection con = dbf.CreateConnection() )
				{
					...
					using ( IDbCommand cmd = con.CreateCommand() )
					{
						cmd.CommandText = sSQL;
						Security.Filter(cmd, "$modulename$", "list");
						Sql.AppendParameter(cmd, prefixText, Sql.SqlFilterMode.StartsWith, "$firsttextfield$");
						cmd.CommandText += " order by $firsttextfield$" + ControlChars.CrLf;
